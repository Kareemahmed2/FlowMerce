...
public class CheckoutService {
    ...
    public CheckoutSummary processCheckout(String email, CartDTOs.CheckoutRequest request) {
        ...
        ShoppingCart cart = cartRepository
                .findByCustomer_CustomerIdAndStore_StoreId(customer.getCustomerId(), store.getStoreId())
                .orElse(null);

        // INT-11: if the server cart is missing or empty, reconcile from client-supplied items.
        // This handles the guest-cart case: customer added items before login, so the
        // server cart is empty but the frontend sends the items in the request body.
        boolean needsReconcile = (cart == null || cart.getItems().isEmpty())
                && request.getItems() != null
                && !request.getItems().isEmpty();

        if (needsReconcile) {
            log.info("INT-11: server cart empty — reconciling {} client items for customer {}",
                    request.getItems().size(), email);
            CartDTOs.AddToCartRequest addReq = new CartDTOs.AddToCartRequest();
            for (CartDTOs.CheckoutRequest.CartItemRequest clientItem : request.getItems()) {
                addReq.setProductId(clientItem.getProductId());
                addReq.setQuantity(clientItem.getQuantity());
                cartService.addItem(email, addReq);
            }
            // Re-fetch the now-populated cart
            cart = cartRepository
                    .findByCustomer_CustomerIdAndStore_StoreId(customer.getCustomerId(), store.getStoreId())
                    .orElseThrow(() -> new BadRequestException("Cart reconciliation failed."));
        }

...
public class PaymentServiceImpl {
    ...
    /** INT-18: list all payments for a store, merchant-only. */
    @Transactional(readOnly = true)
    public List<PaymentDTOs.PaymentResponse> getStorePayments(Integer storeId, String merchantEmail) {
        return paymentRepository.findByOrder_Store_StoreIdOrderByCreatedAtDesc(storeId)
                .stream()
                .map(this::toResponse)
                .toList();
    }

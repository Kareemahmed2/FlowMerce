...
public class ErrorResponse {
    ...
    // CON-2: serialized as "fieldErrors" to match the frontend ApiErrorResponse contract
    // (types/api.types.ts). The only producer is the validation handler, which fills this
    // with per-field messages.
    @JsonProperty("fieldErrors")
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    private Map<String, Object> details;

    // CON-2: ISO-8601 so the frontend can parse it with new Date(...).
    @Builder.Default

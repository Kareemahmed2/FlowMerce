...
public class OrderController {
    ...
    // POST /orders/{orderId}/reorder — INT-14: re-add all items to the buyer's cart
    @PostMapping("/{orderId}/reorder")
    @PreAuthorize("hasRole('BUYER')")
    public ResponseEntity<ApiResponse<CartDTOs.CartResponse>> reorder(
            Principal principal,
            @PathVariable Integer orderId) {
        var items = orderService.getOrderItemsForReorder(principal.getName(), orderId);
        CartDTOs.CartResponse cart = null;
        for (CartDTOs.AddToCartRequest req : items) {
            cart = cartService.addItem(principal.getName(), req);
        }
        return ResponseEntity.ok(ApiResponse.ok(cart, "Items added to cart for reorder"));
    }

    // POST /orders/{orderId}/cancel  (state change — not a deletion)
    ...
    // GET /orders/admin/all?page=0&size=20&sort=orderDate,desc
